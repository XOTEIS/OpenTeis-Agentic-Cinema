...
def build_openteis_prime():
    ...
    src_path = os.path.join(BASE_DIR, "openteis_prime.c")
    so_path = os.path.join(BASE_DIR, "libopenteis_prime.so")
    
    with open(src_path, "w") as f:
        f.write(c_code)
        
    os.system(f"clang -O3 -shared -fPIC {src_path} -o {so_path}")
    print("[✔] OpenTeis Prime C-kjerne (libopenteis_prime.so) er fullstendig kompilert!")

build_openteis_prime()
