    ...
    SharedRingBuffer* shm_hash_create() {
    ...
    int shm_hash_free(SharedRingBuffer* shm) {
        if (shm) return munmap((void*)shm, sizeof(SharedRingBuffer));
        return -EINVAL;
    }

    int shm_hash_push(SharedRingBuffer* shm, const uint8_t* src_data) {
    ...
    int shm_hash_pop_and_digest(SharedRingBuffer* shm, uint8_t* dest_data, uint64_t* out_hash) {
        ...
        *out_hash = compute_fnv1a_64(dest_data, PACKET_SIZE);
